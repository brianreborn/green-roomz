...
**Usefulness bar largely met** for security-monitor hot path on the 8600 GT. `native/sm11-monitor` (CUDA 6.5 / sm_11, **Win32** VS2013 fallback): copy+cmp, FNV (+batch), scrub, seq `{hi,lo}`, fixed private-slot ring (`--ring-push`/`--ring-hash`/`--ring-scrub`), `--loops` / warm `--serve`. JS `sm11-gpu.mjs` wires fat-hash + **`preferRing`** hot path (enqueue/drop/drain → ring assists when `--serve` is up) with scrub/seq/batch probe fallback; ring persists across serve cmds. Load/soak: `scripts/sm11-load.mjs`, `scripts/sm11-soak.mjs`. Place/respond/logger stay off CUDA. **#17 N-API wontfix** — keep `--serve`. Stretch (not bar): CUDA-owned ring index; #16 Win32→x64; #14 optional 0.5B. See `native/sm11-monitor/README.md`, `docs/fleet-targets.md` ship section, issue #13.
